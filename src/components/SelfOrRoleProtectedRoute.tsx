...
interface SelfOrRoleProtectedRouteProps {
  children: React.ReactNode;
  allowedRoles: string[];
}

export const SelfOrRoleProtectedRoute: React.FC<SelfOrRoleProtectedRouteProps> = ({ children, allowedRoles }) => {
  ...
  // Allow access if:
  // 1. The user has one of the allowed roles, OR
  // 2. The user is trying to view their own profile (the ID in the URL matches their userId)
  const isSelfProfile = params.id === user.id;
  const hasAllowedRole = allowedRoles.includes(user.role);
  
  if (!hasAllowedRole && !isSelfProfile) {
    return (
